...
public abstract class CatalogAssembler extends BaseAssembler implements Assembler<JmsCatalogue, CatalogueType> {
    public static CatalogAssembler instance() {
        return Mappers.getMapper(CatalogAssembler.class);
    }

    @Override
    @Mapping(target="domainObject.books", source="books")
    ...
    public abstract JmsCatalogue toDomain(CatalogueType jaxbObject);

    @Override
    @Mapping(target="header.dateOfPreparation", ignore=true)
    @Mapping(target="header.timeOfPreparation", ignore=true)
    @InheritInverseConfiguration
    public abstract CatalogueType toMessage(JmsCatalogue domainObject);
    ...
    @AfterMapping
    protected void setTimestamp(@MappingTarget JmsCatalogue domainObject, CatalogueType jaxbObject) {
        domainObject.setTimestamp(getTimestamp(jaxbObject.getHeader()));
    }

    @AfterMapping
    protected void setDateAndTime(@MappingTarget CatalogueType jaxbObject, JmsCatalogue domainObject) {
